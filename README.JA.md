...
Getopt::EX::Hashed - Getopt::Long のためのハッシュ格納オブジェクト自動化
...
パラメータ数が偶数でない場合、先頭に既定ラベルがあると見なされます。最初のパラメータがコードリファレンスなら `action`、それ以外なら `spec` です。
...
- \[ **spec** => \] _string_
    ...
    特別な指定が不要な場合は、値として空文字列（または空白のみの文字列）を与えてください。そうでない場合、それはオプションとは見なされません。
...
- **default** => _value_ | _coderef_
    ...
    値が ARRAY または HASH へのリファレンスの場合、同じ要素を持つ新しいリファレンスが割り当てられます。これは、メンバーデータが複数の `new` 呼び出し間で共有されることを意味します。`new` を複数回呼び出してメンバーデータを変更する場合は注意してください。
...
- \[ **action** => \] _coderef_
    ...
    `"<>"` で全てを受け取る用途に使えます。その場合、spec パラメータは関係なく、不要です。
...
新しいハッシュオブジェクトを作成するクラスメソッド。すべてのメンバーをデフォルト値で初期化し、設定に従ってアクセサメソッドを作成します。キーがロックされた（LOCK\_KEYS が有効な場合）bless されたハッシュリファレンスを返します。
...
ハッシュキーは `Hash::Util::lock_keys` によって保護されているため、存在しないメンバーへアクセスするとエラーになります。使用前に新しいメンバーキーを宣言するため、この関数を使用してください。
...
オブジェクトを作成する前にクラスメソッド `Getopt::EX::Hashed->configure()` を使用してください。この情報は呼び出し元パッケージ固有の領域に保存されます。`new()` 呼び出し後、パッケージ固有の設定はオブジェクトにコピーされ、以降の操作で使用されます。オブジェクト固有の設定を更新するには `$obj->configure()` を使用します。

利用可能な設定パラメータは次のとおりです。

- **LOCK\_KEYS** (default: 1)

    ハッシュキーをロックします。これにより、存在しないハッシュエントリへの誤アクセスを防ぎます。

- **REPLACE\_UNDERSCORE** (default: 1)

    アンダースコアをダッシュに置き換えた別名を生成します。

- **REMOVE\_UNDERSCORE** (default: 0)

    アンダースコアを削除した別名を生成します。
...
- **DEFAULT**
    ...
    デフォルトパラメータを設定します。`has` が呼ばれると、引数パラメータの前に DEFAULT パラメータが挿入されます。したがって、両方に同じパラメータが含まれている場合は、引数リストの後ろのものが優先されます。`+` を伴うインクリメンタルな呼び出しには影響しません。
