...
Getopt::EX::Hashed - Getopt::Long을 위한 해시 저장 객체 자동화
...
매개변수의 개수가 짝수가 아니면, 맨 앞에 기본 라벨이 있다고 가정합니다: 첫 번째 매개변수가 코드 참조라면 `action`, 그렇지 않으면 `spec`.
...
- \[ **spec** => \] _string_
    ...
    특별한 것이 필요 없다면 값으로 빈 문자열(또는 공백만 있는 문자열)을 주십시오. 그렇지 않으면 옵션으로 간주되지 않습니다.
...
- **default** => _value_ | _coderef_
    ...
    값이 ARRAY 또는 HASH에 대한 참조인 경우, 동일한 멤버를 가진 새 참조가 할당됩니다. 이는 멤버 데이터가 여러 번의 `new` 호출 간에 공유됨을 의미합니다. `new`를 여러 번 호출하고 멤버 데이터를 변경하는 경우 주의하세요.
...
- \[ **action** => \] _coderef_
    ...
    `"<>"`에 대해 모든 것을 포착하는 데 사용할 수 있습니다. 이 경우 spec 파라미터는 무관하며 필요하지 않습니다.
...
새 해시 객체를 생성하는 클래스 메서드입니다. 모든 멤버를 기본값으로 초기화하고 설정에 따라 접근자 메서드를 생성합니다. 잠긴 키(LOCK\_KEYS가 활성화된 경우)를 가진 축복된 해시 참조를 반환합니다.
...
해시 키가 `Hash::Util::lock_keys`에 의해 보호되므로, 존재하지 않는 멤버에 접근하면 오류가 발생합니다. 사용 전에 새 멤버 키를 선언하려면 이 함수를 사용하세요.
...
객체를 생성하기 전에 클래스 메서드 `Getopt::EX::Hashed->configure()`를 사용하세요. 이 정보는 호출 패키지 고유 영역에 저장됩니다. `new()` 호출 후에는 패키지 고유 설정이 객체로 복사되어 이후 동작에 사용됩니다. 객체 고유 설정을 업데이트하려면 `$obj->configure()`를 사용하세요.

다음 구성 매개변수를 사용할 수 있습니다.

- **LOCK\_KEYS** (default: 1)

    해시 키를 잠급니다. 이는 존재하지 않는 해시 항목에 대한 실수 접근을 방지합니다.

- **REPLACE\_UNDERSCORE** (default: 1)

    밑줄을 대시로 바꾼 별칭을 생성합니다.
...
- **DEFAULT**
    ...
    기본 매개변수를 설정합니다. `has`가 호출되면, DEFAULT 매개변수는 인수 매개변수보다 먼저 삽입됩니다. 따라서 둘 다 동일한 매개변수를 포함하면, 인수 목록에서 나중에 온 것이 우선합니다. `+`를 사용하는 증가 호출에는 영향을 주지 않습니다.
