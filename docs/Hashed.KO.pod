...
C<is> 매개변수가 주어지면 접근자 메서드가 자동으로 생성됩니다. 동일한 함수가 이미 정의되어 있으면 프로그램은 치명적 오류를 발생시킵니다. 기본적으로, 객체가 파괴될 때 접근자는 패키지 네임스페이스에서 제거됩니다. 이는 단일 스크립트 내에서 동일한 클래스를 여러 번 사용할 수 있게 해줍니다 — 예를 들어, 해시 기반 객체를 사용하는 모듈이 여러 곳에서 호출되는 경우에 그렇습니다.

복사로 생성된 객체(예: L<Clone>을 통해)는 접근자의 소유자로 간주되지 않습니다. 원래 C<new>로 생성된 객체만이 파괴 시 접근자를 제거하므로, 복제된 객체는 원래 객체나 그 클래스에 영향을 주지 않고 안전하게 파괴될 수 있습니다. 파괴 후에도 접근자를 유지하려면 C<REMOVE_ACCESSOR>를 false로 설정하십시오.
...
원래 C<new>로 생성된 객체만이 접근자의 소유자로 간주됩니다. 복사로 생성된 객체(예: L<Clone>을 통해)는 소유자가 아니므로, C<REMOVE_ACCESSOR>가 true인 경우에도 복제본을 파괴해도 접근자가 제거되지 않습니다.
