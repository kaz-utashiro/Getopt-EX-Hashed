...
C<is> パラメータが与えられると、アクセサメソッドは自動的に生成されます。同名の関数がすでに定義されている場合、プログラムは致命的エラーになります。デフォルトでは、オブジェクトが破棄されるとアクセサはパッケージの名前空間から削除されます。これにより、同一のクラスを1つのスクリプト内で複数回使用できます（たとえば、ハッシュベースのオブジェクトを用いるモジュールが複数箇所から呼び出される場合）。

コピーによって作成されたオブジェクト（例：L<Clone> 経由）はアクセサの所有者とは見なされません。C<new> によって最初に作成されたオリジナルのオブジェクトのみが、破棄時にアクセサを削除します。そのため、クローンを破棄しても元のオブジェクトやそのクラスに影響を与えることはありません。破棄後もアクセサを保持したい場合は、C<REMOVE_ACCESSOR> を偽に設定してください。
...
C<new> によって最初に作成されたオブジェクトのみがアクセサの所有者と見なされます。コピーによって作成されたオブジェクト（例：L<Clone>）は所有者ではないため、C<REMOVE_ACCESSOR> が真であっても、クローンを破棄してもアクセサは削除されません。
