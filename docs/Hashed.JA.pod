...
C<is> パラメータが与えられると、アクセサメソッドが自動的に生成されます。同じ関数がすでに定義されている場合、プログラムは致命的なエラーを引き起こします。オブジェクトが破棄された後もアクセサは残ります。破棄時にそれらを削除したい場合は、C<REMOVE_ACCESSOR> 構成パラメータを true に設定してください。
