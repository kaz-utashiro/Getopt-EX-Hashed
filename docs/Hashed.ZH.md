...
**Getopt::EX::Hashed** 是一个自动创建哈希对象的模块，用于存储 **Getopt::Long** 和兼容模块（包括 **Getopt::EX::Long**）的命令行选项值。模块名称共享 **Getopt::EX** 前缀，但到目前为止，它独立于 **Getopt::EX** 中的其他模块运行。

该模块的主要目标是将初始化和规范整合到一个地方。它还提供了一个简单的验证接口。
...
如果给出的是数组引用，可以同时声明多个名称。

    has [ 'left', 'right' ] => ( spec => "=i" );

如果名称以加号（`+`）开头，则所给参数将更新现有设置。

    has '+left' => ( default => 1 );

至于 `spec` 参数，如果它是第一个参数，则可以省略标签。

    has left => "=i", default => 1;

如果参数个数不是偶数，则假定头部存在默认标签：如果第一个参数是代码引用，则使用 `action`，否则使用 `spec`。
...
- **is** => `ro` | `rw`
    ...
    如果要为以下所有成员创建访问器，请使用 `configure` 设置 `DEFAULT` 参数。

        Getopt::EX::Hashed->configure( DEFAULT => [ is => 'rw' ] );

    如果不喜欢可分配访问器，可将 `ACCESSOR_LVALUE` 参数设置为 0。 因为访问器是在 `new` 时生成的，所以该值对所有成员都有效。

- **default** => _value_ | _coderef_

    设置默认值。如果没有给出缺省值，则成员初始化为 `undef`。

    如果该值是对 ARRAY 或 HASH 的引用，则会分配一个具有相同成员的新引用。这意味着成员数据将在多次调用 `new` 时共享。如果多次调用 `new` 并更改成员数据，请务必小心。

    如果给定了代码引用，则会在调用 **new** 时调用它以获取默认值。当您想在执行时而不是在声明时评估值时，这种方法非常有效。如果要定义默认操作，请使用 **action** 参数。如果要将代码引用设置为初始值，则必须指定一个返回代码引用的代码引用。
...
- \[ **action** => \] _coderef_
    ...
    您可以将此用于 `"<>">> 来捕获所有内容。在这种情况下，spec 参数并不重要，也不是必需的。`
...
以下参数均用于数据验证。首先，`must` 是一个通用验证器，可以实现任何功能。其他参数是常用规则的快捷方式。

- **must** => _coderef_ | \[ _coderef_ ... \]

    参数 `must` 需要一个代码引用来验证选项值。它接收与 `action` 相同的参数，并返回一个布尔值。在下面的示例中，选项 **--answer** 只接受 42 作为有效值。
...
- **any** => _arrayref_ | qr/_regex_/ | _coderef_
    ...
    设置有效字符串参数列表。每个项目可以是字符串、regex 引用或代码引用。当参数与给定列表中的任何一项相同或匹配时，该参数即为有效参数。如果参数值不是数组反射，则会被视为单项列表（通常是 regexpref 或 coderef）。
...
创建新散列对象的类方法。使用默认值初始化所有成员，并根据配置创建访问方法。如果启用了 LOCK\_KEYS，则返回带锁定键的哈希引用。
...
`GetOptions` 可以通过将哈希引用作为第一个参数在哈希中存储值，但这并非必要。
...
由于哈希键受 `Hash::Util::lock_keys` 保护，访问不存在的成员会导致错误。在使用前，请使用此函数声明一个新的成员键。
...
- **ACCESSOR\_PREFIX** (default: '')
    ...
    指定后，它将作为成员名的前缀，使其成为访问方法。如果 `ACCESSOR_PREFIX` 被定义为 `opt_`，成员 `file` 的访问器将是 `opt_file`。

- **ACCESSOR\_LVALUE** (default: 1)

    如果为 true，读写访问器将具有 lvalue 属性。如果不喜欢这种行为，请设置为零。

- **DEFAULT**

    设置默认参数。调用 `has` 时，DEFAULT 参数会插入到参数之前。因此，如果两个参数都包含相同的参数，在参数列表中排在后面的参数优先。使用 `+` 的递增调用不受影响。

    DEFAULT 的一个典型用法是 `is` 为下面所有哈希条目准备访问方法。声明 `DEFAULT => []` 以重置。
